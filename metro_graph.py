...
class MetroGraph:
    ...
    POPULAR_STATIONS = [
        ...
        "Hauz Khas", "Huda City Centre", "AIIMS", "INA", "Dwarka Sector 21",
        "Noida City Centre", "Botanical Garden", "Mandi House", "Lajpat Nagar"
    ]
    ...
    def _initialize_metro_network(self) -> None:
        ...
        all_lines = yellow_line + blue_line + blue_line_branch + red_line + green_line + green_line_branch + violet_line + magenta_line + pink_line + grey_line # Removed direct_connections
        
        # Store line information
        self.lines = {
            'Yellow': [station for station1, station2, _ in yellow_line for station in [station1, station2]],
            'Blue': [station for station1, station2, _ in blue_line for station in [station1, station2]],
            'Blue Branch': [station for station1, station2, _ in blue_line_branch for station in [station1, station2]],
            'Red': [station for station1, station2, _ in red_line for station in [station1, station2]],
            'Green': [station for station1, station2, _ in green_line for station in [station1, station2]],
            'Green Branch': [station for station1, station2, _ in green_line_branch for station in [station1, station2]],
            'Violet': [station for station1, station2, _ in violet_line for station in [station1, station2]],
            'Magenta': [station for station1, station2, _ in magenta_line for station in [station1, station2]],
            'Pink': [station for station1, station2, _ in pink_line for station in [station1, station2]],
            'Grey': [station for station1, station2, _ in grey_line for station in [station1, station2]],
        }
    ...
    def find_shortest_path(self, start: str, end: str) -> Tuple[List[str], float, List[dict]]:
        ...
        try:
            ...
            # Optimized Dijkstra's using min-heap (priority queue)
            ...
            logger.info(f"Route found: {len(path)} stations, {total_distance} km, {len(line_changes)} changes")
    ...
    def find_route_least_changes(self, start: str, end: str) -> Tuple[List[str], float, List[dict]]:
        """
        ...
        try:
            ...
            while pq:
                ...
                for neighbor, edge_dist in self.graph[current].items():
                    ...
                    heapq.heappush(pq, (new_changes, new_dist, neighbor, new_path, new_lines))
    ...
    def _identify_line_changes(self, path: List[str]) -> List[dict]:
        ...
        for i, station in enumerate(path):
            ...
            common_lines = current_lines & station_lines
    ...
    def get_route_summary(self, path: List[str], distance: float, line_changes: List[dict]) -> dict:

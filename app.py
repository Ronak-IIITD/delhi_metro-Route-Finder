    ...
    try:
        ...
        if route_type == "all":
            ...
            for route in routes:
                ...
                summary = metro_network.get_route_summary(
                    route.path, route.distance, route.line_changes
                )
            ...
            if route_type == "least_changes":
                path, distance, line_changes = metro_network.find_route_least_changes(
                    source, destination
                )
            else:
                path, distance, line_changes = metro_network.find_shortest_path(
                    source, destination
                )
...
def _get_station_lines(path):
    ...
    for station in path:
        ...
        station_lines.append({"station": station, "lines": lines})
